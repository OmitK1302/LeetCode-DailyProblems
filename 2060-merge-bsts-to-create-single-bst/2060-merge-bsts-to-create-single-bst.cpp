...
public:
    ...
    // Try to recursively merge leaf nodes
    bool dfsMerge(TreeNode* node, unordered_map<int, TreeNode*>& roots) {
        if (!node) return true;

        // If this node is a leaf and a merge target exists
        if (!node->left && !node->right && roots.count(node->val)) {
            ...
            roots.erase(node->val); // mark this tree as used
        }
    ...
    TreeNode* canMerge(vector<TreeNode*>& trees) {
        ...
        // Step 1: Build root map and record child node values
        ...
        // Step 2: Identify the true root (not a child of any tree)
        ...
        if (!root) return nullptr; // no valid root found

        // Step 3: Merge trees recursively
        if (!dfsMerge(root, roots)) return nullptr;

        // Step 4: All trees must be merged (only root should remain)
        if (roots.size() != 1) return nullptr;

        // Step 5: Validate final tree is a BST
